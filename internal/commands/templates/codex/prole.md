...
**Rule of thumb:** After every file edit, your next action should be
`git add` + `git commit` + `git push`. Every. Single. Time.
...
2. **Accept the assignment**: `gt agent accept <id>`. Do this BEFORE any git
   operation so your agent status flips to `working` immediately.
...
3. **Move the ticket to in_progress** (new work only — skip for `repairing`):
   ...
   `gt agent accept`, before any branch operation.
