...
// --- showClosed filter toggle tests ---
...
func TestShowClosed_zeroTimeCutoffIncludesAll(t *testing.T) {
	...
	m.showClosed = false
	withoutClosed := m.flatTickets()
	if len(withoutClosed) != 0 {
		t.Errorf("expected stale closed node hidden when showClosed=false, got %d nodes", len(withoutClosed))
	}

	m.showClosed = true
	withClosed := m.flatTickets()
	if len(withClosed) != 1 {
		t.Errorf("expected stale closed node shown when showClosed=true, got %d nodes", len(withClosed))
	}
}

// --- status change input tests ---
...
func TestCreateTicket_emptyTitleNoOp(t *testing.T) {
	...
	// Press C then immediately Enter (empty title)
	updated, _ := m.Update(tea.KeyMsg{Type: -1, Runes: []rune("C")})
	dm := updated.(dashboardModel)
	upd, _ := dm.Update(tea.KeyMsg{Type: tea.KeyEnter})
	dm = upd.(dashboardModel)
	...
	if len(after) != beforeCount {
		t.Errorf("expected no new ticket on empty title, but count went from %d to %d", beforeCount, len(after))
	}
	_ = dm
}
